import type { QuestionnaireConfig } from "../types"

export const hausnotrufConfig: QuestionnaireConfig = {
  id: "hausnotruf",
  title: "Hausnotruf",
  description: "erhältlich ab 0€ mit Pflegegrad",
  steps: [
    {
      id: "product",
      title: "Welchen Hausnotruf möchten Sie?",
      description: "Wählen Sie das passende Produkt für Ihre Bedürfnisse",
      questions: [
        {
          id: "product",
          type: "product_selection",
          label: "Produkt",
          required: true,
          products: [
            {
              value: "medivato",
              brandName: "Medivato",
              brandLogo: "/images/logos/medivato-logo.png",
              productName: "Medivato Home",
              productImage: "/images/products/medivato-home.png",
              description: "Stationärer Hausnotruf mit Sturzsensor",
              features: [
                "Sturzsensor mit automatischer Alarmierung",
                "Hochwertige Careium Basisstation",
                "Zugang zum Medivato Gesundheitsclub",
              ],
              price: "ab 0,00€",
              priceUnit: "/ Monat",
              priceNote: "kostenfrei mit Pflegegrad",
            },
            {
              value: "johanniter",
              brandName: "Johanniter",
              brandLogo: "/images/logos/johanniter-logo.png",
              productName: "Johanniter Hausnotruf",
              productImage: "/images/products/johanniter-hausnotruf.png",
              description: "Bewährtes Hausnotrufsystem mit Schlüsselhinterlegung",
              features: [
                "Schlüsselhinterlegung bei den Johannitern",
                "Direkter Kontakt zu Rettungsdiensten",
                "24/7 Notrufzentrale",
              ],
              price: "ab 15€",
              priceUnit: "/ Monat",
              priceNote: "Eigenanteil mit Pflegegrad",
            },
          ],
        },
      ],
    },
    {
      id: "location",
      title: "Wo wird der Hausnotruf benötigt?",
      description: "",
      questions: [
        {
          id: "location",
          type: "radio",
          label: "Nutzungsort",
          required: true,
          options: [
            { value: "Zuhause", label: "Zuhause" },
            { value: "Unterwegs", label: "Unterwegs" },
            { value: "Zuhause und Unterwegs", label: "Zuhause & Unterwegs" },
          ],
        },
      ],
    },
    {
      id: "anfrage-fuer",
      title: "Für wen ist die Anfrage gedacht?",
      questions: [
        {
          id: "anfrage_fuer",
          type: "radio",
          label: "Anfrage für",
          required: true,
          options: [
            { value: "Für mich", label: "Für mich" },
            { value: "Angehörige", label: "Angehörige" },
            { value: "Patient/Kunde", label: "Patient/Kunde" },
            { value: "Andere Person", label: "Andere Person" },
          ],
        },
      ],
    },
    {
      id: "notification",
      title: "Wer soll im Notfall benachrichtigt werden?",
      questions: [
        {
          id: "notification",
          type: "radio",
          label: "Benachrichtigung",
          required: true,
          options: [
            { value: "Angehörige", label: "Angehörige" },
            { value: "Notrufzentrale", label: "Notrufzentrale" },
            { value: "Beide", label: "Beide" },
          ],
        },
      ],
    },
    {
      id: "keyStorage",
      title: "Ist eine Schlüsselhinterlegung erwünscht?",
      questions: [
        {
          id: "keyStorage",
          type: "radio",
          label: "Schlüsselhinterlegung",
          required: true,
          options: [
            { value: "Ja, beim Notruf-Anbieter", label: "Beim Notruf-Anbieter" },
            { value: "Ja, beim Nachbarn", label: "Beim Nachbarn" },
            { value: "Ja, in einer Schlüsselbox", label: "In einer Schlüsselbox" },
            { value: "Nein", label: "Keine Schlüsselhinterlegung" },
          ],
        },
      ],
    },
    {
      id: "pflegegrad",
      title: "Ist ein Pflegegrad vorhanden?",
      questions: [
        {
          id: "pflegegrad",
          type: "radio",
          label: "Pflegegrad",
          required: true,
          options: [
            { value: "Vorhanden", label: "Vorhanden" },
            { value: "Beantragt", label: "Beantragt" },
            { value: "Nicht vorhanden", label: "Nicht vorhanden" },
          ],
        },
      ],
    },
    {
      id: "zeitpunkt",
      title: "Wann wird der Hausnotruf benötigt?",
      questions: [
        {
          id: "zeitpunkt",
          type: "radio",
          label: "Zeitpunkt",
          required: true,
          options: [
            { value: "Sofort", label: "Sofort" },
            { value: "1 Monat", label: "In 1 Monat" },
            { value: "1-3 Monate", label: "In 1-3 Monaten" },
          ],
        },
      ],
    },
    {
      id: "remarks",
      title: "Weitere Anmerkungen zum Kunden/Sachverhalt",
      description: "",
      questions: [
        { id: "anmerkungen", type: "long-text-field", label: "Anmerkungen", placeholder: "Optionale Anmerkungen...", required: false },
      ],
    },
    {
      id: "contact",
      title: "Ihre Kontaktdaten",
      description: "Damit wir Sie für ein unverbindliches Angebot kontaktieren können",
      questions: [
        { id: "anrede", type: "radio", label: "Anrede", required: false, options: [{ value: "Herr", label: "Herr" }, { value: "Frau", label: "Frau" }] },
        { id: "vorname", type: "text", label: "Vorname", placeholder: "Max", required: true },
        { id: "nachname", type: "text", label: "Nachname", placeholder: "Mustermann", required: true },
        { id: "email", type: "email", label: "E-Mail (optional)", placeholder: "max@beispiel.de", required: false },
        { id: "telefonnummer", type: "tel", label: "Telefonnummer", placeholder: "+49 123 456789", required: true },
        { id: "strasse", type: "text", label: "Straße", placeholder: "Musterstraße", required: true },
        { id: "hausnummer", type: "text", label: "HNr.", placeholder: "1a", required: true },
        { id: "plz", type: "text", label: "PLZ", placeholder: "12345", required: true },
        { id: "ort", type: "text", label: "Ort", placeholder: "Musterstadt", required: true },
      ],
    },
    {
      id: "consent",
      title: "Zusammenfassung & Einwilligung",
      description: "Bitte überprüfen Sie Ihre Angaben und stimmen Sie der Datenverarbeitung zu",
      questions: [
        { id: "summary_display", type: "summary", label: "Zusammenfassung", required: false },
        { id: "datenschutz", type: "consent", label: "Datenschutz Einwilligung", required: true },
      ],
    },
  ],
  successMessage: {
    title: "Vielen Dank für Ihre Anfrage!",
    description: "Wir haben Ihre Hausnotruf-Anfrage erhalten und werden uns in Kürze bei Ihnen melden.",
  },
}
